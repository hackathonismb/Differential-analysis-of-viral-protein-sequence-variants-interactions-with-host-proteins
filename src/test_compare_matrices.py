...
def random_test():
    """ Docstring for random_test.
    Run a test on random matrices with low filling rate about N/3 for NxN
    matrices with added noise 10 to 20%.
    
    TODO:
    alternatives:  store the norm of the translation, in the current
    implementation it should be 0 ...

    increase level of noise until norm becomes too large ...
    ...
    name1 = 'random_test_ground_truth_matrix'
    name2 = 'noise_plus_random_test_ground_truth'
    ...
    for i in range(100):
        ...
        m_ground = ( m0 > th_p  ).astype(type(1))
        #print('m_ground.sum().sum()')
        number_of_ones__ground =  m_ground.sum().sum()
        print( '# number_of_ones = ', number_of_ones__ground )
        print('#  n = ',n)
        filling_ratio = number_of_ones__ground / float(n*n)
        print('#  filling ratio f = ',filling_ratio)
        print('#  filling ratio f*n = ',n*filling_ratio)

        noise0 = np.random.rand(n,n)
        noise_i = noise0 > th_noise

        m = (m_ground+noise_i) > 0
        m = m.astype(type(1))
        number_of_ones__m =  m.sum().sum()

        bound_max_score = area_factor * min( number_of_ones__m, number_of_ones__ground )
        best_score, best_translation = bound_max_score, point(0,0)
        #if not (mnz1 == mnz2).all():
        if not np.array_equal( m_ground, m ):
            list_1 = convert_matrix_to_list_of_points( m_ground )
            list_2 = convert_matrix_to_list_of_points( m )
            #max_score_lists = min( list_1.nnz, list_2.nnz )
            max_score_lists = area_factor*min( len(list_1), len(list_2) )
            if bound_max_score != max_score_lists :
                print('#  Warning, mismath of max_scores. Check your data ...')
        ...
        norms.append(best_translation.norm())

    # TODO: generate multiple repetitions and store
    #
    #  TODO:  plot histogram of norms and scores
    #

    output_path ='./test_comparative_alignment/'
    if not os.path.exists(output_path):
        os.makedirs(output_path)
